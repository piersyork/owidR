...
When running this line of code around 10 datasets about the internet are returned. Let's use the dataset with the title: Share of the population using the Internet. The corresponding chart_id to this data is: "Share of the population using the Internet". Using the chart_id as an argument to `owid()` imports that data into R, asigning it to an object called internet. We use the rename argument to give the value column a shorter a clean name.
...
#, summarise = FALSE, filter = c("United Kingdom", "Spain", "Russia", "Egypt", "Nigeria")
owid_plot(democracy) +
...
Now that we have a combined dataset we can get to the analysis. We'll create two models, one with just internet use and democracy, and the other with the confounders added.

```{r analysis, results='asis'}
fe_model <- plm(internet_use ~ polity, data, 
                effect = c("individual"), index = "entity")

fe_model_2 <- plm(internet_use ~ polity + gdp + gov_exp + age_dep + unemp, data, 
                  effect = c("individual"), index = "entity")
